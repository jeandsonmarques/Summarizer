...
def test_bar_slots_accept_category_value_legend_tooltip_and_filters():
    assert binding_slot_names("bar") == ["x_axis", "y_axis", "values", "legend", "tooltip", "filters"]
    assert suggest_binding_slot("bar", "dimension", DashboardChartBinding(chart_type="bar")) == "x_axis"
...
def test_extended_model_visual_types_have_builder_slots():
    ...
    assert suggest_binding_slot("kpi", "measure", DashboardChartBinding(chart_type="kpi")) == "values"
    assert suggest_binding_slot("gauge", "dimension", DashboardChartBinding(chart_type="gauge")) == ""
    assert DashboardChartBinding(chart_type="kpi", measure_field="volume").normalized().has_minimum_fields()
    ...
    for chart_type in (
        ...
        assert binding_slot_names(chart_type) == ["x_axis", "y_axis", "values", "legend", "tooltip", "filters"]
...
def test_incomplete_visuals_have_specific_empty_messages():
    ...
    assert empty_binding_message("bar", DashboardChartBinding(chart_type="bar")) == "Arraste campos para Eixo X/Categoria e Valores."
